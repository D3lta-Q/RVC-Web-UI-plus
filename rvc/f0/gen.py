...
class Generator(object):
    ...
    def calculate(
        ...
        f0_max = 1100
        if f0_method == "pm":
            if not hasattr(self, "pm") or self.pm.hop_length != hop_length:
                from .pm import PM

                self.pm = PM(hop_length, f0_min, f0_max, self.sr)
            f0 = self.pm.compute_f0(x, p_len=p_len)
        elif f0_method == "dio":
            if not hasattr(self, "dio") or self.dio.hop_length != hop_length:
                from .dio import Dio

                self.dio = Dio(hop_length, f0_min, f0_max, self.sr)
            f0 = self.dio.compute_f0(x, p_len=p_len)
        elif f0_method == "harvest":
            if not hasattr(self, "harvest") or self.harvest.hop_length != hop_length:
                from .harvest import Harvest

                self.harvest = Harvest(hop_length, f0_min, f0_max, self.sr)
            f0 = self.harvest.compute_f0(x, p_len=p_len, filter_radius=filter_radius)
        elif f0_method == "crepe":
            if not hasattr(self, "crepe") or self.crepe.hop_length != hop_length:
