...
def extract_f0_feature(n_p, f0method, if_f0, exp_dir, version19, hop_length=128):
    ...
    if if_f0:
        if f0method != "rmvpe_gpu":
            cmd = (
                '"%s" infer/modules/train/extract_f0_print.py "%s/logs/%s" %s %s "%s" %s %s'
                % (
                    config.python_cmd,
                    now_dir,
                    exp_dir,
...
def _parse_loss_log(exp_dir1: str):
    """
    Read logs/<exp_dir1>/train.log and return four DataFrames for the
    live loss graphs. Returns (None, None, None, None) if not ready yet.
    """
    if not exp_dir1:
        return None, None, None, None
    log_path = os.path.join(now_dir, "logs", exp_dir1, "train.log")
    if not os.path.isfile(log_path):
        return None, None, None, None

    steps, disc, gen, fm, mel, kl = [], [], [], [], [], []
    try:
        with open(log_path, "r", encoding="utf-8", errors="replace") as fh:
            for line in fh:
                ...
                if m:
                    ...
                    disc.append(float(m.group("loss_disc")))
                    gen.append(float(m.group("loss_gen")))
                    fm.append(float(m.group("loss_fm")))
                    mel.append(float(m.group("loss_mel")))
                    kl.append(float(m.group("loss_kl")))
    except OSError:
        return None, None, None, None
    ...
    # Disc + Gen share one graph — use long format so Gradio can colour by metric
    df_disc_gen = pd.DataFrame({
        "step":   steps + steps,
        "value":  disc + gen,
        "metric": ["loss_disc"] * len(steps) + ["loss_gen"] * len(steps),
    })
    df_fm  = pd.DataFrame({"step": steps, "value": fm})
    df_mel = pd.DataFrame({"step": steps, "value": mel})
    df_kl  = pd.DataFrame({"step": steps, "value": kl})
    return df_disc_gen, df_fm, df_mel, df_kl


def refresh_loss_plots(exp_dir1: str):
    """Gradio Timer callback — re-reads train.log and returns updated plot data."""
    return _parse_loss_log(exp_dir1)
...
with gr.Blocks(title="RVC WebUI") as app:
    ...
    with gr.Tabs():
        ...
        with gr.TabItem(i18n("Train")):
            ...
            # ── Live Loss Monitor ──────────────────────────────────────────────
            gr.Markdown("### 📈 Live Training Loss Monitor")
            gr.Markdown(
                "_Graphs refresh automatically every 5 seconds while training. "
                "Requires Gradio ≥ 4.0._"
            )
            with gr.Row():
                plot_disc_gen = gr.LinePlot(
                    label="Discriminator & Generator Loss (loss_disc / loss_gen)",
                    x="step",
                    y="value",
                    color="metric",
                    height=260,
                ...
                plot_fm = gr.LinePlot(
                    label="Feature Matching Loss (loss_fm)",
                    x="step",
                    y="value",
                    height=260,
                )
            with gr.Row():
                plot_mel = gr.LinePlot(
                    label="Mel Spectrogram Loss (loss_mel)",
                    x="step",
                    y="value",
                    height=260,
                )
                plot_kl = gr.LinePlot(
                    label="KL Divergence Loss (loss_kl)",
                    x="step",
                    y="value",
                    height=260,
                )

            # Timer polls every 5 s; starts when training kicks off
            loss_timer = gr.Timer(value=5, active=False)
            loss_timer.tick(
                fn=refresh_loss_plots,
                inputs=[exp_dir1],
                outputs=[plot_disc_gen, plot_fm, plot_mel, plot_kl],
            )
            but3.click(
