...
hop_length = int(sys.argv[6]) if len(sys.argv) > 6 else 128
            ...
            for idx, (inp_path, opt_path1, opt_path2) in enumerate(paths):
                try:
                    ...
                    coarse_pit, feature_pit = self.f0_gen.calculate(
                        x, x.shape[0] // self.hop, 0, f0_method, None, hop_length=self.hop
                    )
                    np.save(
                        opt_path2,
                        feature_pit,
...
if __name__ == "__main__":
    ...
    if "cuda" in device:
        ...
        n_p = 1
    featureInput = FeatureInput(is_half, device, hop_size=hop_length)
    paths = []
